...
describe('configuration helpers', () => {
	...
	it('resolves contained workspace paths and refuses a real outside sibling', () => {
		...
		try {
			...
			expect(configHelpers.containedPath(root, scratch.path)).toBe(false)
			expect(configHelpers.workspacePath(scratch.path)).toBeUndefined()
			expect(configHelpers.isOutsideWorkspacePath(scratch.path)).toBe(true)
	...
	it('reads bounded files and resolves package roots from real manifests', () => {
		...
		try {
			...
			scratch.write('node_modules/@sample/package/package.json', '{"name":"@sample/package"}')
			scratch.write('node_modules/@sample/package/src/index.ts', 'export {}\n')
			const under = resolve(workspace, 'under.txt')
			const at = resolve(workspace, 'at.txt')
			const over = resolve(workspace, 'over.txt')
			scratch.write('under.txt', 'abc')
			scratch.write('at.txt', 'abcd')
			scratch.write('over.txt', 'abcde')
	...
	it('drives both plugins through their real Vite hooks', async () => {
		...
		try {
			...
			scratch.write('index.ts', 'export const control = true\n')
			await expect(
				build({
					root,
					configFile: false,
